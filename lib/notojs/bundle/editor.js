window.connection = new class {
    ...
    #packages = Promise.resolve({scripts: [], modules: []});
    #packageLoad = null;
    ...
    pkg() {
        const load = fetch(`${this.#server}p`, {method: 'PROPFIND'})
            .then(response => {
                if(!response.ok) throw new Error(`Failed to load packages: ${response.status}`);
                return response.json();
            })
            .then(packages => ({
                scripts: Array.isArray(packages?.scripts)
                    ? packages.scripts.filter(path => typeof path === 'string') : [],
                modules: Array.isArray(packages?.modules)
                    ? packages.modules.filter(path => typeof path === 'string') : []
            }));
        this.#packageLoad = load;
        return load.then(packages => {
            if(this.#packageLoad === load) {
                this.#packages = Promise.resolve(packages);
                this.#packageLoad = null;
            }
            return packages;
        }).catch(_ => {
            if(this.#packageLoad === load) this.#packageLoad = null;
            return this.#packages;
        });
    }

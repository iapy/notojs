...
CodeMirror.notojs.complete = class Completion {
    ...
    static listen() {
        ...
        document.addEventListener("notoui.Opened", Completion.#opened);
        document.addEventListener("notoui.Closed", Completion.#closed);
    }
    ...
    static #closed(event) {
        for(const cm of Completion.#editors.get(event.detail?.book) || []) {
            Completion.#imports.delete(cm);
            Completion.#indexes.delete(cm);
        }
    }

    static #opened(event) {
        console.log('Opened', event.detail);
        if(!event.detail?.reload)
            return;
        Completion.#paths = null;
        Completion.#pathLoad = null;
        for(const cm of Completion.#editors.get(event.detail?.book) || []) {
            Completion.#imports.delete(cm);
            Completion.#indexes.delete(cm);
            Completion.#getPaths(cm);

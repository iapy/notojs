...
CodeMirror.notojs.edit = class Edit {
    static extraKeys = {
        "'{'": Edit.#autoClose('{', '}'),
        "'('": Edit.#autoClose('(', ')'),
        "'['": Edit.#modeSwitch,
    ...
    static init(cm) {
        ...
        cm.on('cursorActivity', Edit.#macroMatch);
        cm.on('inputRead', Edit.#normalizeHeading);
        cm.on('blur', Edit.#clearMatches);
        Edit.#macroMatch(cm);
    }
    ...
    static #inMarkdownFence(cm, line) {
        let start = line - 1;
    ...
    static #normalizeHeading(cm, change) {
        ...
        if(!heading || Edit.#inMarkdownFence(cm, cursor.line)) return;

        cm.replaceRange(heading[1] + ' ' + heading[2], {line: cursor.line, ch: 0}, {line: cursor.line, ch: line.length}, '+input');
        cm.setCursor(cursor.line, cursor.ch + 1);
    }

    static #macroMatch(cm) {
        Edit.#clearMatches(cm);

        const cursor = cm.getCursor();
        const line = cm.getLine(cursor.line);
        const open = /^<\[(?:!?(?:[A-Za-z_$][A-Za-z0-9_$]*)?|:)\[$/.test(line);
        const close = line === ']]>';
        if(/^<\[:?$/.test(line)) Edit.#clearBracketMatch(cm);

        if(open || close) {
            Edit.#clearBracketMatch(cm);
            ...
            Edit.#markLines(cm, from, to, 'cm-notojs-matchingmacro');
            return;
        }

        if(/^\s*```/.test(line)) {
            const opening = Edit.#fenceCount(cm, cursor.line) % 2 == 0;
            let from = cursor.line;
            let to = cursor.line;
            if(opening) {
            ...
            Edit.#markLines(cm, from, to, 'cm-notojs-matchingfence');
        }
    }

    static #clearMatches(cm) {
        cm.notojs = cm.notojs || {};
    ...
    static #markLines(cm, from, to, className) {
        cm.notojs = cm.notojs || {};
        cm.notojs.edit = cm.notojs.edit || {};
        cm.notojs.edit.macroMarks = [
    ...
    static #autoClose(o, c) {
        return cm => {
            if(!Edit.#active(cm)) return CodeMirror.Pass;
    ...
    static #enter(cm) {
        ...
        const state = cm.getTokenAt(cursor).state;
        const line = cm.getLine(cursor.line);
        if(state?.inMarkdown) return Edit.#markdownEnter(cm, cursor);
        if(state?.inTemplate && line[cursor.ch - 1] === '`' && /^\)*\s*$/.test(line.slice(cursor.ch))) {
            const base = line.match(/^\s*/)[0];
            const next = Edit.#nextText(cm, cursor.line);
            if(Edit.#hasTemplateClose(cm, cursor.line)) {
    ...
    static #hasTemplateClose(cm, line) {
        for(let i = line + 1; i < cm.lineCount(); ++i) {
            const text = cm.getLine(i).trim();
    ...
    static #markdownEnter(cm, cursor) {
        let line = cm.getLine(cursor.line);
        const heading = line.match(/^(\s{0,3}#{1,6})([^#\s].*)$/);
        if(heading && !Edit.#inMarkdownFence(cm, cursor.line)) {
        ...
        const nextFence = Edit.#nextText(cm, cursor.line)?.match(/^(`{3,})\s*$/);
        if(fence && Edit.#fenceCount(cm, cursor.line) % 2 == 0 &&
    ...
    static #closeSquare(cm) {
        ...
        return Edit.#close(']')(cm);
    }

    static #closeMacro(cm) {
        if(cm.somethingSelected()) return CodeMirror.Pass;
    ...
    static #modeSwitch(cm) {
        ...
        if(!/^<\[(?:!?(?:[A-Za-z_$][A-Za-z0-9_$]*)?|:)\[$/.test(text)) return Edit.#autoClose('[', ']')(cm);
        if(line.slice(cursor.ch).trim()) return CodeMirror.Pass;
    ...
    static #semicolon(cm) {
        ...
        const cursor = cm.getCursor();
        const line = cm.getLine(cursor.line);
        if(Edit.#forHeader(cm, cursor)) return CodeMirror.Pass;

...
export const Handlers = {
    ...
    "notojs.Markdown": (grid, part) => {
        ...
        html.querySelectorAll('li code').forEach(code => {
            const expression = code.textContent.trim();
            const match = /^doc\('([^'\r\n]+)'\)$/.exec(expression);
            const item = code.closest('li');
            const cell = code.closest('nj-cell');
            if(!match || !item || !cell || item.textContent.trim() !== expression) return;

            const query = match[1];
            item.classList.add('nj-doc-reference');
            item.onclick = () => {
                cell.next(`import doc from 'noto:doc';
print(doc(${JSON.stringify(query).replaceAll('"', "'")}));`);
            };
        });

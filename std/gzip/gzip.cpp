...
class GZipError : public bridge::Exception<GZipError>
{
    std::error_code ec;
public:
    GZipError(std::error_code &&ec): ec{ec} {}
...
{
    return (v = noto::Blob::make(ctx, std::move(data))), Resolve;
}
...
struct GZip : bridge::Interface<GZip, std::pair<std::filesystem::path, bool>>
{
    ...
    JSValue blob(JSContext *ctx)
    {
        ...
        if(std::filesystem::is_regular_file(ref().first, ec))
            ...
            return GZipError::throw_(ctx, std::move(ec));
        else
            return GZipError::throw_(ctx, std::make_error_code(std::errc::no_such_file_or_directory));
    }
    ...
    {
        if(!ref().second) return GZipError::throw_(ctx, std::make_error_code(std::errc::read_only_file_system));
...
{
    if(!stream.is_open()) return GZipError::throw_(ctx, std::make_error_code(std::errc::no_such_file_or_directory));
    return std::make_shared<Write<std::string>>(std::move(stream), std::move(data))->run(ctx);
}

template<>
JSValue GZip::writer<noto::Blob>(JSContext *ctx, std::ofstream &&stream, noto::Blob &&data)
{
    if(!stream.is_open()) return GZipError::throw_(ctx, std::make_error_code(std::errc::no_such_file_or_directory));
...
JSValue gzip(JSContext *ctx, noto::fs::Path path)
{
...
JSModuleDef *js_init_module(JSContext *ctx, const char *name)
{
    ...
    JS_AddModuleExport(ctx, mod, GZipError::name());

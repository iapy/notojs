...
class ZipError : public bridge::Exception<ZipError>
{
    std::variant<std::error_code, std::string> ec;
public:
    ZipError(std::string &&ec): ec{ec} {}
    ZipError(std::error_code &&ec): ec{ec} {}
...
public:
    ...
    Then then(JSContext *ctx, JSValue &v) override
    {
        if(!handle)
        {
            v = ZipError::reject_(ctx, std::make_error_code(std::errc::bad_file_descriptor));
            return Reject;
        }
        else
        {
            ...
            for(auto &[k, v]: entries)
                obj.set(k.c_str(), noto::File::make(ctx, std::move(v), k, 0));
            v = obj;
            return Resolve;
        }
    }
...
struct Zip : bridge::Interface<Zip, std::pair<std::filesystem::path, bool>>
{
    ...
    {
        if(!std::filesystem::exists(ref().first)) return ZipError::throw_(ctx, std::make_error_code(std::errc::no_such_file_or_directory));
        std::unordered_set<std::string> entries;
    ...
    {
        if(!std::filesystem::exists(ref().first)) return ZipError::throw_(ctx, std::make_error_code(std::errc::no_such_file_or_directory));
        if(!ref().second) return ZipError::throw_(ctx, std::make_error_code(std::errc::read_only_file_system));
    ...
    JSValue write_0(JSContext *ctx, bridge::Object data)
    {
        ...
        {
            if(noto::fs::Path::check(ctx, *it))
            {
                auto [path, _] = noto::fs::Path{ctx, **it}->native();
                if(!std::filesystem::exists(path)) return ZipError::throw_(ctx, "File doesn't exist at key " + std::string{it.key()});
                entries.insert(std::make_pair(it.key(), std::move(path)));
            }
            else if(noto::Blob::check(ctx, *it))
            {
                auto copy = noto::Blob{ctx, **it}->copy();
            ...
            else return ZipError::throw_(ctx, "Unsuported entry type at key " + std::string{it.key()});
        }
    ...
    JSValue toJSON(JSContext *ctx) const
    {
        ...
        if(!handle) return ZipError::throw_(ctx, std::make_error_code(std::errc::bad_file_descriptor));
...
JSModuleDef *js_init_module(JSContext *ctx, const char *name)
{
    ...
    JS_AddModuleExport(ctx, mod, ZipError::name());
